...
def reflex2change(infile: str, outfile: str, keep_oov: bool=False):
    ...
    for idx, row in indf.iterrows():
        ...
        num_edges = 0  # count total number of edges in a sound correspondence
        for reflex in reflexes:
            ...
            for reflex in reflex.split('/'):
                ...
                try:
                    short_paths = find_shortest_paths(proto, reflex)
                ...
                if "stage1" not in outfile:
                    # to further deduplicate, only take the one with the longest path
                    longest_path = max([len(p) for p in short_paths])
                    short_paths = [path for path in short_paths if len(path) == longest_path]
                    # remove edges where proto == reflex since Chacon doesn't count them
                    short_paths = short_paths[0:1]

                    if longest_path > 1:
                        num_shortest_paths.append(len(short_paths))
                        len_shortest_paths += [len(path) - 1 for path in short_paths]
                else:
                    # get all of the unique phones
                    phones = set()
                    for path in short_paths:
                        phones.update(path)
                    # sort paths by length, descending order
                    short_paths.sort(key=lambda path: len(path), reverse=True)
                    # check that a new path contributes a new phone
                    curr_covered_phones = set()
                    good_paths = []
                    for path in short_paths:
                        if set(path) - curr_covered_phones:
                            good_paths.append(path)
                            curr_covered_phones.update(path)
                    short_paths = good_paths
        ...
        print(num_edges, end=' ', flush=True)
        if num_edges > 1000:
            BIG_NUMS.append(num)
    print()

    print(sum(num_edges_list) / len(num_edges_list), 'avg # of edges / sound corr.')
    print(sum(num_shortest_paths) / len(num_shortest_paths), 'avg # shortest paths')
    print(max(num_shortest_paths), 'max # shortest paths')
    print(min(num_shortest_paths), 'min # shortest paths')
    print(sum(len_shortest_paths) / len(len_shortest_paths), 'avg # edges / proto->reflex path')
